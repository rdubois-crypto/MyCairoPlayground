...
func test_full_windowed{range_check_ptr, ec_op_ptr: EcOpBuiltin* } ()->(res:felt){
 alloc_locals;
 ...
 let res:EcPoint=ec_mulmuladd_W(G,Q,scalar_u, scalar_v); 
 let res_naive:EcPoint=ec_mulmuladd_naive(G,Q,scalar_u, scalar_v); 

// let resp:EcPoint=G;
 
 if(res.x!=res_naive.x){
 let cmp=0;
 return (res=cmp);
 }
 let cmp=1;
...
func main{range_check_ptr, ec_op_ptr: EcOpBuiltin* }() {
    ...
    let (__fp__, _) = get_fp_and_pc();
     ...
     %{ print("\n*******************CAIRO:Shamir's trick+Windowing testing over Stark Curve") %}
    
    
    test_full_windowed{range_check_ptr=range_check_ptr, ec_op_ptr=ec_op_ptr}();
